...
/*#define VISION_SDMC_CD_N_TO_SYS PM8058_GPIO_PM_TO_SYS(VISION_GPIO_SDMC_CD_N)*/
...
static unsigned int vision_sdslot_status(struct device *dev)
{
	...
	status = (unsigned int) gpio_get_value(VISION_SDMC_CD_N_TO_SYS);
...
static struct mmc_platform_data vision_sdslot_data = {
	...
	.status_irq	= MSM_GPIO_TO_INT(VISION_SDMC_CD_N_TO_SYS),
...
int vision_wifi_power(int on)
{
	...
	/* vision_wifi_bt_sleep_clk_ctl(on, ID_WIFI); */
